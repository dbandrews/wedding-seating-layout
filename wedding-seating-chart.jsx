...
export default function SeatingPlanner() {
  const [seats, setSeats] = useState(buildInitial);
  const [pos, setPos] = useState(() =>
    Object.fromEntries(TABLE_META.map((t) => [t.id, { x: t.x, y: t.y }]))
  );
  ...
  const [meta, setMeta] = useState({ couple: "Andrews and Stuart", date: "" });
  ...
  return (
    ...
    >
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Bodoni+Moda:opsz,wght@6..96,400;6..96,600&family=Karla:wght@400;500;700&display=swap');
...
function ExportPanel({
  ...
  return (
    <div>
      ...
      <p className="text-xs leading-relaxed mt-4 mb-2" style={{ color: C.muted }}>
        CSV files open straight into Excel or Google Sheets. If Download is blocked in this preview,
        use Copy and paste into a blank sheet. Changes live in this session only — export before you
        close the tab.
      </p>
    </div>
  );
